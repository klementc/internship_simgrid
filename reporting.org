...
- Meeting notes
  ...
  parts
  When saying that clouds are complex, say that so many problems are NP complete
  ...
  contributions are (tell simgrid): modelization of workload, implementation of
  an API, evaluation
  ...
  on start of contribution tell what is the characterization of a workload (what
  is a task/gridlet/cloudlet (a constant computing load) which doesn't match a
  fluctuating reactive cloud workload)
  we're proposing a discrete representation of a continous event (one argument
  is that a simulator is discrete)
