...
- to detect threshold we would need to set an alarm and be killed if we don't
  ...
  time used) (the online way we can talk about it in conclusion as we didn't do
  it) (the callback is defined by the user)
- say in introduction why I did this internship
  say in conclusion what I learned
